"""A workflow: a project's transitions, taken in the order it wrote them down.

The rung above `halyard.transitions`, the way a transition is the rung above a
command. A transition hands one reply on and stops there; somebody at a phone
reads what came back and presses the next one. A workflow presses it: each step
is one of the project's transitions going to one of its seats, and the reply's own
