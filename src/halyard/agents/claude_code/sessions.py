...
class SessionRef:
    ...
    #: Whether the name was chosen by a person or generated by Claude. It
    #: matters because only one of them is stable: a generated title is
    #: rewritten as the conversation moves, so a seat configured against one
    #: routes correctly today and silently stops when the title changes. The
    #: listing you copy a name from has to be able to say which is which.
    named_by_a_person: bool = False
