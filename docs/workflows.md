  ...
  alpha-engine:
    ...
    workflows:
      steps:
        to_nav:       {seat: nav}
        review:       {seat: xreview}
        reviewed:     {seat: nav, decided_by: review}
        discover:     {handoff: driver_discover, seat: xdrv}
        discover_glm: {handoff: driver_discover, seat: zdrv}
        discovered:   {handoff: discover_completed, seat: nav}
        develop:      {handoff: driver_develop, seat: xdrv}
        develop_glm:  {handoff: driver_develop, seat: zdrv}
        close:        {seat: nav}
      level3:    [review, reviewed, discover, discovered, develop, close]
...
is then a list of step names, in order; `steps` is the one name under `workflows:` that
is not a workflow.
...
When the seat a step went to replies, Halyard reads the last line of that reply. The
words are fixed:

- `DECISION: forward` takes the next step, carrying the reply to it;
- `DECISION: back` sends the work to the step before — the one that produced what was
  just judged — as that step's next round;
- `DECISION: wait` stops the run until somebody sends it on;
- anything else carries on to the next step, and the chat says there was no decision
  line.

A line that is only the word reads the same, and so does one a model put in bold. The
same word in the middle of a paragraph, or after some other label, decides nothing. A
project's prompts only have to ask for the line: every step is told where each word
goes.
...
`decided_by:` on the step after the review: `reviewed` above.
...
```
- Workflow: level3 · step 4 of 6 · discovered
- Decide on your last line: DECISION: forward (→ develop, xdrv) · back (→ discover, xdrv, round 2 of 2) · wait (→ the operator)
```

The review is told where its decision goes, and the navigator what was decided:

```
- Decide on your last line: DECISION: forward or back (→ reviewed, nav, who acts on it) · wait (→ the operator)
```

```
- Workflow: level3 · step 2 of 6 · reviewed
...
A step may go as many times as its `rounds:` allows: two unless it says otherwise,
because going back runs the step before a second time. It counts every time its handoff
has gone for the work item the branch names — pressed by hand as well, since the seat
read it either way — and the envelope's `Round: n/…` counts against the same number. A
step that would go past it does not go: the run stops, and the chat offers **▶️ Send it
anyway** and **⏹ Stop the workflow**.
...
going says where it is — `level3 3/6 · discover — waiting for xdrv` — and offers the same
